...
values will be the professor's favorites. For example, the name Annie has value 
1 + 14 + 14 + 9 + 5 = 43, while the name Earz, though shorter, has value 
5 + 1 + 18 + 26 = 50. 
...
def answer(names):
    return sorted(rev_list(names), 
    	key=lambda name: word_to_val(name), 
    	reverse=True)
...
def alpha_to_num(alpha):
	return ord(alpha.lower()) - 96

def word_to_val(word):
	return sum([alpha_to_num(letter) for letter in word])